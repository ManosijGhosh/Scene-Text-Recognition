...
function [data] = datacreate(n,num)
    ...
    rng('shuffle');
    max=int16(12);%number of features we have
    min=int16(4);%number of features we take minimum
    if(max+min>num)
        max=max-1;
        min=min-1;
    end
    ...
    for i=1:n
        x2=int16(abs(rand*(max-min)))+(min);%number of features we select at max will have 5 features less than maximum
        temp=rand(1,num);
        [~,temp]=sort(temp);
        for j= 1:x2
            data(i,temp(j))=1;
