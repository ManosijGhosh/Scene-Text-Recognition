...
for i = 1:size(CCs,2)
    ...
    ACC = bwconncomp(ShowOutput(:,:,(2*(i-1)+1)));
...
%        if isTextGroup == 1
            ...
            img = ShowOutput(:,:,2*i);
%             for al = 1:aligns
%                 img(ACC.PixelIdxList{aligned_comps(1,al)}) = 1;
%             end
            img(BB(2),BB(1):min(col,BB(1)+BB(3))) = 1;
            img(min(row,BB(2)+BB(4)),BB(1):min(col,BB(1)+BB(3))) = 1;
            img(BB(2):min(row,BB(2)+BB(4)),BB(1)) = 1;
            img(BB(2):min(row,BB(2)+BB(4)),min(col,BB(1)+BB(3))) = 1;
            ShowOutput(:,:,2*i) = img;
